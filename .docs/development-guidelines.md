...
- ESLint・Prettier は未導入。コード品質のチェックは `npx tsc --noEmit` による型チェックで担保する
- インデントはスペース2つ、文字列はシングルクォートを基本とする
...
プレフィックス: `feat` / `fix` / `docs` / `refactor` / `style` / `chore` / `build`

`build` は GitHub Pages 向けビルド出力（`docs/`）のコミットに使用する。
